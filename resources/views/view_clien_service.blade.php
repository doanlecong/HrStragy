...
@section('content')
    ...
    <div class="container-fluid mt-2  position-relative">
        <div class="row background-white padding-bottom-40">
                        ...
                        <ul style="list-style: none" class="no-padding-left">
                            @foreach($clientServices as $service)
                                @if($service->id != $serviceSelect->id)
                                    <li><a href="{{ route('view_type_client_service', $service->slug.".html") }}" class="font-playfair text-20 green-text animate-bottom-nocontent">{{ mb_substr($service->name, 0 ,30) }}{{ strlen($service->name) > 30 ? "...":"" }}</a></li>
                                @endif
                            @endforeach
                        </ul>
                    </nav>
                    ...
                    <nav>
                        <ul style="list-style: none" class="no-padding-left">
                            @foreach($relatePosts as $rel)
                                <li><a href="{{ route('view_client_service', $rel['slug'].".html") }}" class="font-playfair text-18 green-text animate-bottom-nocontent">{{ mb_substr($rel['title'], 0 ,30) }}{{ strlen($rel['title']) > 30 ? "...":"" }}</a></li>
                            @endforeach
                        </ul>
                    </nav>
                </aside>
            ...
            <div class="col-sm-9">
                <section class="border-top-green w-75 " >
