    ...
    <div class="col-lg no-padding-left-right">
        <div class="card card-no-border">
            <div class="card-header card_header_gradient no-border-radius">
                <h3 class="text-left orange-text font-roboto-light"><i class="fa fa-bookmark-o" aria-hidden="true"></i> Chỉnh Sửa Job</h3>
            </div>
            <div class="card-body no-padding-left-right background-blue no-padding-top">
                <div class="row  background-white padding-top-30 border-top-green">
                    <div class="col">
                        ...
                        <form action="{{ route('job.update', $job->id) }}" onsubmit="return validateFormJob();" novalidate enctype="multipart/form-data" autocomplete="off" method="POST">
                            {{ csrf_field() }}
                            @method('PUT')
                            <input type="text" id="currentJobID" hidden value="{{ $job->id }}">
                            <p>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" autocomplete="name" id="name" name="name" value="{{ $job->job_name }}">
                                            ...
                                            <select type="text" class="form-control" id="job_type_id" name="job_type_id">
                                                ...
                                                @foreach($jobTypes as $jobType)
                                                    @if($jobType->id == $job->job_type_id)
                                                        <option value="{{ $jobType->id }}" selected>{{ $jobType->abbr }} -- {{ $jobType->name }}</option>
                                                    @else
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select type="text" class="form-control" id="job_cate_id" name="job_cate_id[]" style="height: 40px" multiple>
                                                <?php $arrayCateSelect = $job->getArrayCateSelected() ?>
                                                @foreach($relateJobCate as $cate)
                                                    @if(in_array($cate->id, $arrayCateSelect))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select type="text" class="form-control" id="job_level_id" name="job_level_id[]" style="height: 40px" multiple>
                                                <?php $arrayLevelSelect = $job->getArrayLevelSelected() ?>
                                                @foreach($relateJobLevel as $level)
                                                    @if(in_array($level->id, $arrayLevelSelect))
                                                        <option value="{{ $level->id }}" selected>{{ $level->abbrie }} -- {{ $level->name }}</option>
                                                    @else
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input id="image" class="form-control" placeholder="Image's Path" type="text" id="image" name="image" value="{{ $job->image }}">
                                        </div>
                                        <img id="holder_1" style="margin-top:15px;max-height:100px;" src="{{ $job->image }}">
                                        <hr>
                                        <p>Choose Time To Active Job Searching In Customer's Page <span class="text-danger">**</span></p>
                                        <div class="input-daterange input-group" id="datepicker">
                                            <input type="text" class="input-sm form-control" id="from" name="time_start" placeholder="Active From" value="{{ date('m/d/Y', strtotime($job->time_from)) }}"/>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" readonly class="form-control" id="slug" name="slug" value="{{ $job->slug }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea type="text" class="form-control" id="description" name="description" rows="4" style="resize: vertical; max-height: 200px;" placeholder="Some text to display on the customer's page ">{{ strip_tags($job->description) }}</textarea>
                                        </div>
                                        <hr>
                                        <div class="form-group">
                                            <label for="salary">Lương (Salary):  <span class="text-danger">**</span></label>
                                            <input type="text" class="form-control" id="salary" name="salary" placeholder="1000$-2000$" title="You can type in the range of salary " value="{{ $job->salary }}">
                                        </div>
                                        <hr>
                                    </div>
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="contentInfo" id="content" class="form-control" rows="10">{{ $job->content }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('scriptTail')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#job_cate_id, #job_level_id, #job_type_id, #country_id, #company_id, #province_id , #district_id').select2({placeholder: "Please Choose"});
            $('#slug').click(function () {
                let val = $(this).val();
                if(val != "") {
