            ...
            <div class="card-body no-padding-left-right background-blue no-padding-top">
                <div class="row  background-white padding-top-30 border-top-green">
                    <div class="col">
                        ...
                        <form action="{{ route('job_level.update', $jobLevel) }}"  enctype="application/x-www-form-urlencoded" autocomplete="off" method="POST">
                            {{ csrf_field() }}
                            @method('PUT')
                            <p>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label for="abbr">Tên Viết Tắt : </label>
                                            <input type="text" class="form-control" name="abbr" required value="{{ $jobLevel->abbrie }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label for="name">Level Name : </label>
                                            <input type="text" class="form-control" name="name" required value="{{ $jobLevel->name }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            @if(!empty($jobtype))
                                                <select readonly type="text" class="form-control" name="job_type_id" required>
                                                    <option selected value="{{ $jobtype->id }}">{{ $jobtype->abbr }} -- {{ $jobtype->name }}</option>
                                                </select>
                                            @elseif(!empty($jobtypes))
                                                <select name="job_type_id" class="form-control" required>
                                                    <option value="">Please choose a job type</option>
                                                    @foreach($jobtypes as $jt)
                                                        <option value="{{ $jt->id }}">{{ $jt->id }} -- {{$jt->name}}</option>
                                                    @endforeach
                                                </select>
                                            @endif
                                        </div>
