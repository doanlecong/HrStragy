        ...
        <div class="card card-no-border">
            <div class="card-header card_header_gradient no-border-radius">
                <h3 class="text-left orange-text font-roboto-light"><i class="fa fa-bookmark-o" aria-hidden="true"></i>
                    Provinces In <a class="animate-bottom-nocontent green-text font-playfair" href="{{ route('country.index') }}">{{ strtoupper($country->name) }}</a></h3>
            </div>
            <div class="card-body no-padding-left-right background-blue no-padding-top">
                <div class="row background-white padding-top-30 border-top-green">
                    <div class="col">
                        <p>
                            <span><a href="{{ route('province.create','country='.$country->id) }}" class="btn btn-primary box-shadown-darkblue"><i
                                            class="fa fa-plus" aria-hidden="true"></i>  Add new province</a></span>
                            <input type="text" value="{{$country->id}}" id="country_id">
                        </p>
                                ...
                                @if(count($provinces) > 0)
                                    @foreach($provinces as $t)
                                        <tr id="row{{$t->id}}">
                                            ...
                                            <td>
                                                {{ $t->districts()->count() }} <a title="Manage Belonged Districts" href="{{ route('district.index','province='.$t->id) }}" class="btn btn-green white-text"><i class="fa fa-bars"></i></a>
                                            </td>
                                            <td id="tdrow{{ $t->id }}">
                                                @if($t->status == config('global.statusActive'))
                                                    <span style="cursor: pointer;" title="Click to show or hide this in customer's page" data-id="{{ $t->id }}" data-href="{{ route('province.toggleShow', $t->id) }}" class="badge background-green white-text toogle-show box-shadown-light-dark">{{ config('global.'.$t->status) }}</span>
                                                @else
                                                    <span style="cursor: pointer;" title="Click to show or hide this in customer's page" data-id="{{ $t->id }}" data-href="{{ route('province.toggleShow', $t->id) }}" class="badge badge-danger white-text toogle-show box-shadown-light-dark">{{ config('global.'.$t->status) }}</span>
                                                @endif
                                            </td>
                                            <td>
