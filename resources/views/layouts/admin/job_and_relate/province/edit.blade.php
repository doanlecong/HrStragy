    ...
    <div class="col-lg no-padding-left-right">
        <div class="card card-no-border">
            <div class="card-header card_header_gradient no-border-radius">
                <h3 class="text-left orange-text font-roboto-light"><i class="fa fa-bookmark-o" aria-hidden="true"></i> Chỉnh Sửa Province Thuộc {{ $province->country->name }}</h3>
            </div>
            <div class="card-body no-padding-left-right background-blue no-padding-top">
                <div class="row  background-white padding-top-30 border-top-green">
                    <div class="col">
                        ...
                        <form action="{{ route('province.update', $province->id) }}"  enctype="application/x-www-form-urlencoded" autocomplete="off" method="POST">
                            {{ csrf_field() }}
                            @method('PUT')
                            <p>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="name" required value="{{ $province->name }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label for="name">Province Code : </label>
                                            <input type="text" class="form-control" name="code" value="{{ $province->code }}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            ...
                                            <select type="text" class="form-control" name="country_id" required readonly>
                                                <option selected value="{{ $province->country->id }}">{{ $province->country->name }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
